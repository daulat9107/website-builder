  ...
  computed: {
    filteredRecords() {
      ...
      for (var i = data.length - 1; i >= 0; i--) {
        ...
        if(!newData.hasOwnProperty(data[i].order_id)){
          newData[data[i].order_id]=[];
        }

        newData[data[i].order_id].push({
          is_active: data[i].status !='completed' ? true: false,
          status: data[i].status_label,
          date: data[i].status_update_date,
  ...
  <div>
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        >
        <div> 
          <template v-for="(data, order_id) in filteredRecords">
            <div class="flex font-bold p-4">Order Id: #{{order_id}}</div>
            <Timeline :items="data" />
          </template>
