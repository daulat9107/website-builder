...
export default defineComponent({
    ...
    methods: {
        ...
        getProductQueryParameters() {
            return queryString.stringify({
                supplier_id: this.creating.form.supplier_id,
            });
        },
        getProductsBySupplierId() {
            axios
                .get(
                    ...
                    }?${this.getProductQueryParameters()}`
                )
                .then((response) => {
                    if (response.data && response.data.data) {
                        this.productsBySupplier = response.data.data;
                    }
                })
                .catch((error) => {
                    alert(error.message);
        ...
        addProduct() {
            this.creating.form.products.push({
                product_id: null,
                unit: null,
                qty: null,
                rate: null,
        ...
        removeProductBYId(id) {
            let i = this.creating.form.products.length;
            while (i--) {
                if (
                    this.creating.form.products[i] &&
                    this.creating.form.products[i].hasOwnProperty(
                        "product_id"
                    ) &&
                    arguments.length > 0 &&
                    this.creating.form.products[i]["product_id"] === id
                ) {
                    this.creating.form.products.splice(i, 1);
                }
            }
        },
        setSubTotalAndTotal() {
            let subtotal = 0;
            let total = 0;
            if (this.creating.form.products.length > 0) {
                subtotal = this.creating.form.products.reduce(
                    (subtotal, product) => {
            ...
            if (this.creating.form.products.length > 0) {
                total = this.creating.form.products.reduce((total, product) => {
                    return total + (product.amount - product.discount);
                }, 0);
            }
            this.creating.form.subtotal = subtotal;
            this.creating.form.total = total;
        },
        calAmount(index) {
            if (this.creating.form.products.length > 0) {
                let price =
                    this.creating.form.products[index].qty *
                    this.creating.form.products[index].rate;
                this.creating.form.products[index].taxable =
                    price -
                    price * (this.creating.form.products[index].gst / 100);
                this.creating.form.products[index].amount = price;
                let gstHalf =
                    (price * (this.creating.form.products[index].gst / 100)) /
                    2;
                this.creating.form.products[index].cgst = gstHalf;
                this.creating.form.products[index].sgst = gstHalf;
            }
        ...
        <div class="panel panel-default">
            <div class="m-4 float-right">
                <a
                    ...
                    >{{ creating.active ? "Hide" : "Place New Order" }}</a
                >
            </div>
            <divider />
            <div
            ...
            >
                <form action="#" @submit.prevent="store">
                    ...
                    >
                        ...
                        >
                            <template v-if="column.name == 'supplier_id'">
                                <field :label="column.label" help="">
                                    <control
                                        ...
                                        @change="getProductsBySupplierId()"
                    ...
                    <divider />
                    <div v-if="creating.form.supplier_id">
                        <button
                        ...
                        <template v-if="this.creating.form.products.length > 0">
                            <div
                                class="flex gap-2 p-4"
                                v-for="(product, index) in this.creating.form
                                    .products"
            ...
            <div class="relative">
                <form action="#" @submit.prevent="getRecords">
                    <div class="flex gap-2 justify-around">
                        <div class="flex-1">
                            <select
                                class="px-3 py-2 max-w-full dark:bg-slate-800 focus:ring focus:outline-none border-gray-700 rounded w-full dark:placeholder-gray-400 dark:text-white"
                                ...
                                >
                                    ...
                                    <template v-else>
                                        <template v-if="column == 'id'">
                                            <Link
                                                :href="`/order-products?order_id=${columnValue}`"
                                                >{{ columnValue }}</Link
                                            >
                                        </template>
                                        <template v-else>
